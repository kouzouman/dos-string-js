...
require('dos-number-js'); // console.log(process.title)
...
_dosCommonJs.default.extendMethod(String, 'changeDecimal', function (integerDigit, decimalDigit, decimalZero) {
  ...
  if (divNum.length > 2) return ''; //  整数部のみの値ですべて数値要素の場合

  if (divNum.length == 1 && target.toInt().toString().length == target.length) {
    //  0埋めを行う場合
    if (decimalZero) return target.substring(0, integerDigit) + (decimalZeroMax == '' ? '' : '.' + decimalZeroMax); //  0梅を行わない場合
...
_dosCommonJs.default.extendMethod(String, 'toBase64', function () {
...
_dosCommonJs.default.extendMethod(String, 'fromBase64', function () {
  // if (process.title !== 'browser') {
